                ...
                <div class="sparkline13-list shadow-reset mg-t-30">
                    <div class="sparkline13-hd">
                        <div class="main-sparkline13-hd">
                            <h1>Service Category Details <span class="table-project-n"></span></h1>
                            ...
                            <table id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true" data-cookie-id-table="saveId" data-show-export="true" data-click-to-select="true" data-toolbar="#toolbar">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$row['id']}}</td>
                                        <td>{{$row['cname']}}</td>
                                        <td><img src="{{asset('/admin/img')}}/{{$row['image']}}" height="50px" width="50px"></td>
                                        <td>{{$row['status']}}</td>
                                        <td>{{$row['created_at']}}</td>
                                        <td>{{$row['updated_at']}}</td>
                                        <td><a class="btn btn-xs btn-primary" href="{{action('CategoryController@edit',$row['id'])}}">Edit</a></td>                    
                                        <td>
                                        <form method="POST" action="{{action('CategoryController@destroy', $row['id'])}}">
                                        @csrf
                                        @method('DELETE')
